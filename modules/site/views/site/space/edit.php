        ...
        <form class="uniform" id="myForm" action="<?php echo url::current(true);?>" method="post" enctype="multipart/form-data">
            <dl class="inline">
                ...
                <dd><input type="text" name="space_name" id="space_name" class="medium required" maxlength="20"
                           value="<?php if(!empty($space_info->name)) {echo $space_info->name;}?>"/></dd>

                <dt><label for="Spacetype"><?php echo __('Spacetype');?><span class="require">*</span></label></dt>
                <dd>
                    <select name="space_type" id="space_type" onChange="change()">
                        <?php foreach ($ads_info as $key=>$ad_info) { ?>
                        <option value="<?php echo $key;?>" <?php echo ($key == $space_info->type)
                                ? 'selected="selected"' : '';?>><?php echo $ad_info['name'];?></option>
                        <?php }?>
                    </select>
                    <div id="t3" style="display:none;">
                    	<input type="checkbox" name="isAlign" value="1" <?php if ($space_info->align == 1) { ?> checked="checked" <?php }?>>&nbsp;<?php echo __('isCenter'); ?>&nbsp;&nbsp;&nbsp;&nbsp;
                	</div>
                	<div id="t4" style="display:none;">
                		<input type="checkbox" name="isScroll" value="1" <?php if ($space_info->scroll == 1) { ?> checked="checked" <?php }?>>&nbsp;<?php echo __('isRollin'); ?>&nbsp;&nbsp;&nbsp;&nbsp;
                	</div>
                </dd>
                ...
                <div id="t1" style="display:none;">
                    ...
                    <dd>
                        <div>
                        <?php echo __('Width');echo '： ';?><input type='text' style='width:50px' name='space_witdh' id='space_witdh' class='medium required' maxlength='10' min="0" value='<?php if(isset($space_info->width)){echo $space_info->width;}?>'/>&nbsp;px&nbsp;&nbsp;&nbsp;
                        </div>
                        <div>
                        <?php echo __('Height');echo '： ';?><input type='text' style='width:50px' name='space_height' id='space_height' class='medium required' maxlength='10' min="0" value='<?php if(isset($space_info->height)){echo $space_info->height;}?>'/>&nbsp;px
                        </div>
                    </dd>
                </div>
                ...
                <div id="t2" style="display:none;">
                    ...
                    <dd>
                        <?php echo __('Paddleft');echo '： ';?><input type='text' style='width:40px' name='padd_left' id='padd_left' class='medium required' maxlength='10' min="0" value='<?php if(isset($space_info->paddleft))echo $space_info->paddleft;?>'/>&nbsp;px&nbsp;&nbsp;&nbsp;
                        <?php echo __('Paddtop');echo '： ';?><input type='text' style='width:40px' name='padd_top' id='padd_top' class='medium required' maxlength='10' min="0" value='<?php if(isset($space_info->paddtop))echo $space_info->paddtop;?>'/>&nbsp;px
                    </dd>
                </div>
                
                <dt><label for="space_description"><?php echo __('Space Description');?></label></dt>
                <dd><textarea id="space_description" name="space_description" class="big" type="textarea" ><?php isset($space_info->description) && print($space_info->description);?></textarea></dd>
            </dl>
            <div class="buttons">
