...
class EHOVEL {
    ...
    public static function config($group)
    {
        ...
        {
            $value = EHOVEL::model('Site_Config')->getc($group);
        }
        if (!empty($value)) {
            return $value;
        } else {
    ...
    public static function upload_url($uri, $size = NULL)
    {
        ...
        if ($base_url === NULL) {
            $base_url = (array)EHOVEL::config('upload.base_url');
            $rand_max = count($base_url) - 1;
        }
        //默认图片
        if(empty($uri))
        {
            $uri = EHOVEL::config('product.product_image_default');
        }
        ...
        if (!empty($uri)) {
            ...
            // 仅有图片允许 $size 参数
            if (!in_array(strtolower(pathinfo($uri, PATHINFO_EXTENSION)), EHOVEL::config('upload.image_formats'))) {
                $size = NULL;
            }
...
/**
    ...
    public static function register($key, $value)
    {
        ...
        {
            //不允许重复注册全局变量及修改值
            throw new Kohana_Exception(__('registry key "'.$key.'" already exists'));
        }
